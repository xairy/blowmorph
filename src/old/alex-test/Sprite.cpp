...
bool Sprite::Init(bm::Texture* texture, size_t tileIndex) {
...
void Sprite::Render() {  
  ...
  glRotatef(angle / M_PI * 180.0f, 0.0, 0.0, 1.0);
  
  glm::uvec2 size = texture->GetTileSize(tileIndex);
  glm::uvec2 tilePos = texture->GetTilePosition(tileIndex);
...
glm::uvec2 Sprite::GetSize() const {
  return texture->GetTileSize(tileIndex);
}
